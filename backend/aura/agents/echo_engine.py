...
def _load(fname: str) -> list[str]:
    p = LEXICON_DIR / fname
    if not p.exists():
        print(f"[ECHO] WARNING: lexicon file not found: {p}")
        return []
    return [
        line.strip().lower()
...
# Uses a plain dict — safe for single-process uvicorn (reload=True)
_sessions: dict[str, list[float]] = {}


# ══════════════════════════════════════════════════════════════════════
# STAGE 1 — INTERACTION ANALYZER: 19-dimensional feature vector
# ══════════════════════════════════════════════════════════════════════
def extract_features(text: str, session_id: str, turn: int) -> dict:
    ...
    # F6-F8: Valence, Arousal, Dominance
    ...
    attachment_words = ["only you", "always here", "understand me",
                        "nobody else", "only one"]
    alpha_att = _phrase_score(t, attachment_words)

    # F11: Hedging reduction (1.0 = no hedging = decisive/panicking)
    hedge_words = ["maybe", "perhaps", "i think", "might",
                   "could be", "possibly", "not sure"]
    ...
    # F13: Crisis flag (hardcoded override — intentional per blueprint)
    ...
    reassurance_words = ["is this ok", "am i safe",
                         "will i be ok", "should i be worried"]
    ...
    return {
        ...
        "intent_dependency":  intent_dependency,
        "intent_inquiry":     intent_inquiry,
        "intent_panic":       intent_panic,
        ...
        "flag_jailbreak":     flag_jailbreak,
        "flag_critical":      flag_critical,
        "harm_keyword_density": harm_density,
        "trend_distress":     trend_distress,
...
def compute_risk(
    ...
    # Append to session history AFTER computing (so this turn's score
    # doesn't influence its own context_score)
...
def _rule_score(f: dict) -> float:
    ...
    # Fixed threshold: manual spec says 0.08, old code had 0.15
...
# ══════════════════════════════════════════════════════════════════════
def classify_risk(
    composite: float, flag_critical: float
) -> tuple[RiskLevel, CalibrationMode, list[str]]:
    if flag_critical == 1.0:
        return (
            RiskLevel.CRITICAL,
            CalibrationMode.CRISIS_REDIRECT,
            ["CRISIS PHRASE DETECTED — redirecting to emergency services immediately"],
        )
    if composite >= settings.RISK_HIGH:
        return (
            RiskLevel.HIGH,
            CalibrationMode.FULL_REWRITE,
            ["High panic indicators",
             "Response simplified to 3 steps",
    ...
    if composite >= settings.RISK_MEDIUM:
        return (
            ...
            ["Moderate distress detected",
             "Safety caveats added to response"],
        )
    if composite >= settings.RISK_LOW:
        return (
            RiskLevel.LOW,
            CalibrationMode.PASSTHROUGH,
...
async def score(payload: dict, req: ProcessRequest) -> ECHOResult:
    features = extract_features(
        payload["text"], req.session_id, req.turn_number
    )
...
def _compute_valence(text: str) -> float:
    neg   = ["pain", "hurt", "scared", "dying", "help", "bleeding",
             "cant", "severe", "bad", "terrible"]
...
def _compute_dominance(text: str) -> float:
    helpless = ["i cant", "dont know", "what do i do",
                "lost", "overwhelmed", "no idea", "confused"]
    score = 0.5
